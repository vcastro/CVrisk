#' AHA/ACC PREVENT 2023 30-year ASCVD risk score (BMI-based)
#'
#' Computes 30-year risk for ASCVD event using the PREVENT equations
#' (2023) with BMI instead of lipids. PREVENT is race-neutral and represents
#' the modern replacement for the 2013 Pooled Cohort Equations.
#'
...
#'
#' @return Estimated 30-Y Risk for ASCVD (percent)
#'
#' @export
#'
#' @examples
...
ascvd_30y_prevent_bmi <- function(gender = c("male", "female"),
                                  age, bmi, sbp,
  ...
  prevent_30y_bmi_coef <- NULL
  utils::data(prevent_30y_bmi_coef, envir = environment())
  ...
  model_coef <- merge(sex_df, prevent_30y_bmi_coef)
